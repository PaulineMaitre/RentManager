<%@ page contentType="text/html;charset=UTF-8" %>
<%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix = "c" %>
<!DOCTYPE html>
<html>
<%@include file="/WEB-INF/views/common/head.jsp"%>
<body class="hold-transition skin-blue sidebar-mini">
<div class="wrapper">

    <%@ include file="/WEB-INF/views/common/header.jsp" %>
    <!-- Left side column. contains the logo and sidebar -->
    <%@ include file="/WEB-INF/views/common/sidebar.jsp" %>

    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
        <!-- Content Header (Page header) -->
        <section class="content-header">
            <h1>Réservations
                <a class="btn btn-primary" style="margin-left:10px" href="${pageContext.request.contextPath}/rents/create">
                <i class="fa fa-plus"></i> Ajouter</a>
           </h1>
        </section>

        <!-- Main content -->
        <section class="content">
            <div class="row">
                <div class="col-md-12">
                <p><em><b><i class="fa fa-info"></i> Vous pouvez cliquer sur le nom du véhicule ou du client pour obtenir sa fiche personnelle</b></em></p>
                    <div class="box">
                        <div class="box-body no-padding">
                            <table class="table table-striped">
                                <tr>
                                    <th style="width: 10px">#</th>
                                    <th>Véhicule</th>
                                    <th>Client</th>
                                    <th>Début</th>
                                    <th>Fin</th>
                                    <th>Action</th>
                                </tr>
                                <c:forEach items = "${rents}" var = "rent">
                                <tr>
                                    <td>${rent.id}</td>
                                    <td><a href = "${pageContext.request.contextPath}/vehicles/details?id=${rent.vehicleId.id}"> ${rent.vehicleId.manufacturer} ${rent.vehicleId.model}</a></td>
                                    <td><a href = "${pageContext.request.contextPath}/users/details?id=${rent.clientId.id}"> ${rent.clientId.firstName} ${rent.clientId.lastName}</a></td>
                                    <td>${rent.beginning}</td>
                                    <td>${rent.end}</td>
                                    <td>
                                        <a class="btn btn-success" href="${pageContext.request.contextPath}/rents/edit?id=${rent.id}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger" href="${pageContext.request.contextPath}/rents/delete?id=${rent.id}">
                                            <i class="fa fa-trash"></i>
                                        </a>
                                    </td>
                                </tr>
                                </c:forEach>
                            </table>
                        </div>
                        <!-- /.box-body -->
                    </div>
                    <!-- /.box -->
                </div>
                <!-- /.col -->
            </div>
        </section>
        <!-- /.content -->
    </div>

    <%@ include file="/WEB-INF/views/common/footer.jsp" %>
</div>
<!-- ./wrapper -->

<%@ include file="/WEB-INF/views/common/js_imports.jsp" %>
</body>
</html>
